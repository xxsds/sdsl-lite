...
template <class t_iv>
void test_Constructors(uint8_t   template_width,
					   size_type constructor_size,
					   uint8_t   constructor_width)
{
	...
	{
		// test construction from list with iterators
		std::list<uint64_t> list;
		for (size_t i = 0; i < constructor_size; i++) {
			uint64_t expected_val = rng();
			list.push_back(expected_val);
		}
		t_iv iv(list.begin(), list.end(), constructor_width);
		ASSERT_EQ(constructor_size, iv.size());
		if (iv.fixed_int_width == 0) {
			ASSERT_EQ(constructor_width, iv.width());
		} else {
	...
	{
		// test construction from vector with iterators
		std::vector<uint64_t> vec;
		for (size_t i = 0; i < constructor_size; i++) {
			size_type expected_val = rng();
			vec.push_back(expected_val);
		}
		t_iv iv(vec.begin(), vec.end(), constructor_width);
		ASSERT_EQ(constructor_size, iv.size());
		if (iv.fixed_int_width == 0) {
			ASSERT_EQ(constructor_width, iv.width());
		} else {
	...
	{
		// test construction from vector with object
		std::vector<uint32_t> vec;
		for (size_t i = 0; i < constructor_size; i++) {
			size_type expected_val = rng();
			vec.push_back(expected_val);
		}
		t_iv iv(vec, constructor_width);
		ASSERT_EQ(constructor_size, iv.size());
		if (iv.fixed_int_width == 0) {
			ASSERT_EQ(constructor_width, iv.width());
		} else {
...
TEST_F(IntVectorTest, push_back)
{
	std::mt19937_64 rng;
	for (size_type i = 0; i < vec_sizes.size(); ++i) {
