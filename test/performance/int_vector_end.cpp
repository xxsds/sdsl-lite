...
// g++-11 -std=c++20 -O3 -march=native -DNDEBUG -falign-loops=64 -I/.../sdsl-lite/include/ /.../sdsl-lite/test/performance/int_vector_end.cpp
...
void run_benchmark(std::vector<uint64_t> const & random_values)
{
    ...
    double const mean = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count() / (double) values.size();
