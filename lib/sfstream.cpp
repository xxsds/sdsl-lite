...
osfstream::osfstream() : std::ostream(nullptr) { this->init(m_streambuf); }
...
osfstream::buf_ptr_type osfstream::open(const std::string& file, std::ios_base::openmode mode)
{
	delete m_streambuf;
	m_streambuf				= nullptr;
	m_file					= file;
...
osfstream::operator voidptr() const { return m_streambuf; }

osfstream& osfstream::seekp(pos_type pos)
{
...
osfstream& osfstream::seekp(off_type off, std::ios_base::seekdir way)
{
...
isfstream::isfstream() : std::istream(nullptr) { this->init(m_streambuf); }
...
isfstream::buf_ptr_type isfstream::open(const std::string& file, std::ios_base::openmode mode)
{
	delete m_streambuf;
	m_streambuf				= nullptr;
	m_file					= file;
...
isfstream& isfstream::seekg(pos_type pos)
{
	ios_base::iostate err = std::ios_base::iostate(std::ios_base::goodbit);
	try {
		if (!this->fail()) {
...
isfstream& isfstream::seekg(off_type off, std::ios_base::seekdir way)
{
	ios_base::iostate err = std::ios_base::iostate(ios_base::goodbit);
	try {
		if (!this->fail()) {
...
isfstream::~isfstream() { delete m_streambuf; }
