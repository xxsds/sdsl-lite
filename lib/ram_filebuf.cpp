...
std::streambuf* ram_filebuf::open(const std::string name, std::ios_base::openmode mode)
{
	// open ram_file
...
bool ram_filebuf::is_open() { return m_ram_file != nullptr; }

ram_filebuf* ram_filebuf::close()
{
...
ram_filebuf::pos_type ram_filebuf::seekpos(pos_type sp, std::ios_base::openmode mode)
{
...
ram_filebuf::pubseekoff(off_type off, std::ios_base::seekdir way, std::ios_base::openmode which)
{
...
ram_filebuf::pos_type ram_filebuf::pubseekpos(pos_type sp, std::ios_base::openmode which)
{
...
int ram_filebuf::sync()
{
	return 0; // we are always in sync, since buffer is sink
}

std::streamsize ram_filebuf::xsputn(const char_type* s, std::streamsize n)
{
...
ram_filebuf::int_type ram_filebuf::overflow(int_type c)
{
	if (m_ram_file) {
