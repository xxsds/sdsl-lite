...
ram_fs::ram_fs() { m_fd_map[-1] = ""; }

void ram_fs::store(const std::string& name, content_type data)
{
...
bool ram_fs::exists(const std::string& name)
{
	auto&								  r = ram_fs::the_ramfs();
	std::lock_guard<std::recursive_mutex> lock(r.m_rlock);
...
ram_fs::content_type& ram_fs::content(const std::string& name)
{
	auto&								  r = ram_fs::the_ramfs();
	std::lock_guard<std::recursive_mutex> lock(r.m_rlock);
	return r.m_map[name];
...
size_t ram_fs::file_size(const std::string& name)
{
	auto&								  r = ram_fs::the_ramfs();
	std::lock_guard<std::recursive_mutex> lock(r.m_rlock);
	if (exists(name)) {
...
int ram_fs::remove(const std::string& name)
{
	auto&								  r = ram_fs::the_ramfs();
	std::lock_guard<std::recursive_mutex> lock(r.m_rlock);
	r.m_map.erase(name);
...
int ram_fs::rename(const std::string old_filename, const std::string new_filename)
{
	auto&								  r = ram_fs::the_ramfs();
	std::lock_guard<std::recursive_mutex> lock(r.m_rlock);
...
ram_fs::content_type& ram_fs::content(const int fd)
{
	auto&								  r = ram_fs::the_ramfs();
	std::lock_guard<std::recursive_mutex> lock(r.m_rlock);
...
int ram_fs::truncate(const int fd, size_t new_size)
{
	auto&								  r = ram_fs::the_ramfs();
	std::lock_guard<std::recursive_mutex> lock(r.m_rlock);
...
size_t ram_fs::file_size(const int fd)
{
	auto&								  r = ram_fs::the_ramfs();
	std::lock_guard<std::recursive_mutex> lock(r.m_rlock);
...
int ram_fs::open(const std::string& name)
{
	auto&								  r = ram_fs::the_ramfs();
	std::lock_guard<std::recursive_mutex> lock(r.m_rlock);
	if (!exists(name)) {
...
bool is_ram_file(const int fd) { return fd < -1; }
