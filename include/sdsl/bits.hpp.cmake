...
inline uint64_t bits::map10(uint64_t x, uint64_t c) { return ((x ^ ((x << 1) | c)) & (~x)); }
...
inline uint64_t bits::map01(uint64_t x, uint64_t c) { return ((x ^ ((x << 1) | c)) & x); }
...
inline void bits::write_int(uint64_t* word, uint64_t x, uint8_t offset, const uint8_t len)
{
	x &= bits::lo_set[len];
	if (offset + len < 64) {
		*word &= ((bits::all_set << (offset + len)) | bits::lo_set[offset]); // mask 1..10..01..1
		*word |= (x << offset);
		//		*word ^= ((*word ^ x) & (bits::lo_set[len] << offset) );
		//      surprisingly the above line is slower than the lines above
	} else {
		*word &= ((bits::lo_set[offset])); // mask 0....01..1
		*word |= (x << offset);
...
inline void
bits::write_int_and_move(uint64_t*& word, uint64_t x, uint8_t& offset, const uint8_t len)
{
	x &= bits::lo_set[len];
	if (offset + len < 64) {
		*word &= ((bits::all_set << (offset + len)) | bits::lo_set[offset]); // mask 1..10..01..1
...
inline uint64_t bits::read_int_and_move(const uint64_t*& word, uint8_t& offset, const uint8_t len)
{
	...
	if ((offset = (offset + len)) >= 64) { // if offset+len > 64
		if (offset == 64) {
			...
			offset &= 0x3F;
			return w1 | (((*(++word)) & bits::lo_set[offset]) << (len - offset));
		}
	} else {
		return w1 & bits::lo_set[len];
	}
...
inline uint64_t bits::read_unary(const uint64_t* word, uint8_t offset)
{
	...
	if (w) {
		...
		if (0 != (w  = *(++word))) return bits::lo(w) + 64 - offset;
		uint64_t cnt = 2;
		while (0 == (w = *(++word)))
			++cnt;
