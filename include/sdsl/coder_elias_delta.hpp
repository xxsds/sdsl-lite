...
public:
	...
	// 1 represents 1 and is the code word with minimum length
	...
	static uint64_t
	decode(const uint64_t* d, const size_type start_idx, size_type n, t_iter it = (t_iter) nullptr)
	{
		d += (start_idx >> 6);
		uint64_t  value = 0;
		size_type i		= 0;
	...
	start_decoding:
		...
		while (i < n) { // while not all values are decoded
			...
			decode_single:
				i++;
				uint16_t len_1_len =
				bits::read_unary_and_move(d, offset); // read length of length of x
				uint16_t len_1 =
				bits::read_int_and_move(d, offset, len_1_len) + (1ULL << len_1_len) - 1;
				value +=
				bits::read_int_and_move(d, offset, len_1) + (len_1 < 64) * (1ULL << (len_1));
