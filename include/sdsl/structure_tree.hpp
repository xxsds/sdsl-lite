...
inline void write_structure_tree(const structure_tree_node* v, std::ostream& out, size_t level = 0)
{
	fprintf(stderr, "write_structure_tree: unsupported output format\n");
...
template <>
inline void
write_structure_tree<JSON_FORMAT>(const structure_tree_node* v, std::ostream& out, size_t level)
{
	if (v) {
...
inline std::string create_html_header(const char* file_name)
{
	...
	std::string		  sdsl_dir = SDSL_PROJECT_DIR;
	jsonheader << "<html>\n"
			   << "   <head>\n"
			   << "    <meta http-equiv=\"Content-Type\" content=\"text/html;charset=utf-8\">\n"
			   << "    <title>" << file_name << "</title>\n"
			   << "    <script src=\"file://" << sdsl_dir << "/external/d3/d3.min.js\"></script>"
