...
#if SDSL_HAS_CEREAL
        int_vector<> isa_buf;
        load_from_file(isa_buf, cache_file_name(conf::KEY_ISA, config));
#else
...
#if SDSL_HAS_CEREAL
	store_to_file(lcp_buf, lcp_file);
#else
	lcp_buf.close();
#endif
...
#if SDSL_HAS_CEREAL
	int_vector<> lcp_out_buf;
	lcp_out_buf.width(sa_buf.width());
	lcp_out_buf.resize(n);
#else
...
#if SDSL_HAS_CEREAL
	store_to_file(lcp_out_buf, cache_file_name(conf::KEY_LCP, config));
#else
	lcp_out_buf.close();
#endif
	register_cache_file(conf::KEY_LCP, config);
...
inline void construct_lcp_go(cache_config& config)
{
	...
	// phase 1: calculate lcp_sml;  memory consumption: 2n bytes (lcp_sml=n bytes, text=n bytes)
	{
		...
		{
#if SDSL_HAS_CEREAL
			int_vector<8> bwt_buf;
			load_from_file(bwt_buf, cache_file_name(conf::KEY_BWT, config));
#else
	...
	// phase 2: calculate lcp_big
	{
		...
		1); // lcp_big first contains adapted LF values and finally the big LCP values
		{
			...
			bit_vector todo(n, 0); // bit_vector todo indicates which values are >= m in lcp_sml
			{
				...
				load_from_file(lcp_sml_buf, cache_file_name("lcp_sml", config));
#else
	...
	{
#if SDSL_HAS_CEREAL
		int_vector<> lcp_big_buf;
		load_from_file(lcp_big_buf, cache_file_name("lcp_big", config));
		int_vector<8> lcp_sml_buf;
		load_from_file(lcp_sml_buf, cache_file_name("lcp_sml", config));
		int_vector<> lcp_buf;
		lcp_buf.width(lcp_big_buf.width());
		lcp_buf.resize(n);
#else
...
#if SDSL_HAS_CEREAL
		store_to_file(lcp_buf, cache_file_name(conf::KEY_LCP, config));
#else
		lcp_buf.close();
#endif
	}
...
void construct_lcp_bwt_based2(cache_config& config)
{
...
#if SDSL_HAS_CEREAL
		int_vector<> lcp_positions_buf;
		lcp_positions_buf.width(int_width);
		lcp_positions_buf.resize(n);
#else
...
#if SDSL_HAS_CEREAL
		store_to_file(lcp_positions_buf, tmp_lcp_file);
#else
	...
	// (2) Insert LCP entires into LCP array
	{
...
#if SDSL_HAS_CEREAL
		int_vector<> lcp_positions;
		lcp_positions.width(int_width);
		load_from_file(lcp_positions, tmp_lcp_file);
#else
		...
		std::string lcp_file		 = cache_file_name(conf::KEY_LCP, config);
#if SDSL_HAS_CEREAL
		int_vector<> lcp_array;
		lcp_array.width(int_width);
		lcp_array.resize(n);
		number_of_values = n;
#else
		...
		// Close file
#if SDSL_HAS_CEREAL
		store_to_file(lcp_array, lcp_file);
		register_cache_file(conf::KEY_LCP, config);
