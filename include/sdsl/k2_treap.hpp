...
private:
	...
	uint8_t get_t(const t_tv& v)
	{
		...
		std::begin(v), std::end(v), [&](t_e a, t_e b) { return tupmax(a) < tupmax(b); });
		uint64_t x   = tupmax(*max_it);
		uint8_t  res = 0;
...
public:
	...
	k2_treap(int_vector_buffer<>& buf_x, int_vector_buffer<>& buf_y, int_vector_buffer<>& buf_w)
	{
		using namespace k2_treap_ns;
		typedef int_vector_buffer<>* t_buf_p;
		std::vector<t_buf_p>		 bufs = {&buf_x, &buf_y, &buf_w};
	...
	void construct(std::vector<std::tuple<t_x, t_y, t_w>>& v, std::string temp_file_prefix = "")
	{
		...
		std::string val_file = temp_file_prefix + "_k2_treap_" + id_part + ".sdsl";
		std::string bp_file  = temp_file_prefix + "_bp_" + id_part + ".sdsl";

		{
			...
			for (uint64_t l = t, cc = 0; l + 1 > 0; --l) {
				if (l > 0) {
					m_level_idx[l - 1] = m_level_idx[l] + last_level_nodes;
					m_coord[l - 1] =
					int_vector<>(2 * last_level_nodes, 0, bits::hi(precomp<t_k>::exp(l)) + 1);
				}
				level_nodes = 0;
				cc			= 0;
				auto sp		= std::begin(v);
				...
				for (size_t j = 0; j < t_k; ++j) {
					...
					if (m_bp[v.idx + t_k * i + j]) {
						...
						if (v.t > 1) {
							auto y = rank - m_level_idx[v.t - 1];
							_max_p =
							t_p(_x + m_coord[v.t - 2][2 * y], _y + m_coord[v.t - 2][2 * y + 1]);
						}
